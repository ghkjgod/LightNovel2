...
import com.ghkjgod.lightnovel.service.HeartbeatSessionKeeper;
...
public class LightUserSession {
    ...
    public static Wenku8Error.ErrorCode doLoginFromFile() {
        ...
        byte[] b = LightNetwork.okHttpPostConnection(Wenku8API.getBaseURL(), Wenku8API.getUserLoginParams(username, password));
    ...
    public static Wenku8Error.ErrorCode doLoginFromGiven(String name, String pwd) {
        ...
        byte[] b = LightNetwork.okHttpPostConnection(Wenku8API.getBaseURL(), Wenku8API.getUserLoginParams(name, pwd));
        if(b == null) return Wenku8Error.ErrorCode.NETWORK_ERROR;
        try {
            String result = new String(b, "UTF-8");
            if(!LightTool.isInteger(result)) {
