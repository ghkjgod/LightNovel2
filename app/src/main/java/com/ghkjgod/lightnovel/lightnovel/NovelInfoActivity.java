...
import com.ghkjgod.lightnovel.global.api.ChapterInfo;
import com.ghkjgod.lightnovel.global.api.NovelItemMeta;
import com.ghkjgod.lightnovel.global.api.OldNovelContentParser;
import com.ghkjgod.lightnovel.global.api.VolumeList;
import com.ghkjgod.lightnovel.global.api.Wenku8API;
import com.ghkjgod.lightnovel.global.api.Wenku8Error;
import com.ghkjgod.lightnovel.global.api.Wenku8Parser;
import com.ghkjgod.lightnovel.reader.activity.Wenku8ReaderActivityV1;
...
public class NovelInfoActivity extends AppCompatActivity {
    ...
    private SimpleDraweeView ivNovelCover = null;
    ...
    private void initUI(){
        ...
        ivNovelCover = (SimpleDraweeView) findViewById(R.id.novel_cover);
        ...
        //fresco加载
        Uri uri = Uri.parse(Wenku8API.getCoverURL(aid));
        ivNovelCover.setImageURI(uri);
        tvNovelShortIntro.setVisibility(TextView.GONE);
        ibNovelOption.setVisibility(ImageButton.INVISIBLE);
        fabFavorate.setColorFilter(ContextCompat.getColor(this,R.color.default_white), PorterDuff.Mode.SRC_ATOP);
        fabDownload.setColorFilter(ContextCompat.getColor(this,R.color.default_white), PorterDuff.Mode.SRC_ATOP);
        llCardLayout.setBackgroundResource(R.color.menu_transparent);
        if (GlobalConfig.testInLocalBookshelf(aid)) {
            fabFavorate.setIcon(R.mipmap.ic_favorate_pressed);
        ...
        if (Build.VERSION.SDK_INT >= 16 ) {
            ...
            tvNovelTitle.setBackground(ContextCompat.getDrawable(this, R.drawable.btn_menu_item));
            tvNovelAuthor.setBackground(ContextCompat.getDrawable(this, R.drawable.btn_menu_item));
            // set Navigation bar color
            if(Build.VERSION.SDK_INT >= 21)
                getWindow().setNavigationBarColor(ContextCompat.getColor(this,R.color.myNavigationColor));
...
//                                                        for(int i = 0; i < which.length; i ++) vidList[i] = listVolume.get(which[i]).vid;
        ...
        protected Integer doInBackground(Integer... params) {
            ...
            try {
                if(fromLocal) {
                    ...
                    byte[] byteNovelFullMeta = LightNetwork.okHttpPostConnection(Wenku8API.getBaseURL(), cv);
            ...
            try {
                if(fromLocal) {
                    ...
                    byte[] byteNovelFullInfo = LightNetwork.okHttpPostConnection(Wenku8API.getBaseURL(), cvFullIntroRequest);
            ...
            try {
                if(fromLocal) {
                    ...
                    byte[] byteNovelChapterList = LightNetwork.okHttpPostConnection(Wenku8API.getBaseURL(), cv);
    ...
    class AsyncUpdateCacheTask extends AsyncTask<Integer, Integer, Wenku8Error.ErrorCode> {
        ...
        protected Wenku8Error.ErrorCode doInBackground(Integer... params) {
            ...
            try {
                ...
                byte[] tempVolumeXml = LightNetwork.okHttpPostConnection(Wenku8API.getBaseURL(), cv);
                ...
                byte[] tempIntroXml = LightNetwork.okHttpPostConnection(Wenku8API.getBaseURL(), cv);
                ...
                byte[] tempFullIntro = LightNetwork.okHttpPostConnection(Wenku8API.getBaseURL(), cv);
            ...
            for (VolumeList tempVl : vl) {
                for (ChapterInfo tempCi : tempVl.chapterList) {
                    ...
                    if (xml == null || xml.length() == 0 || operationType == 2) {
                        String Expath = GlobalConfig.getFirstFullSaveFilePath()+"novel";
                        if(!LightNetwork.okHttpDownload(Wenku8API.getBaseURL(), cv,Expath, tempCi.cid + ".xml",true)){
                            Expath = GlobalConfig.getSecondFullSaveFilePath()+"novel";
                            LightNetwork.okHttpDownload(Wenku8API.getBaseURL(), cv,Expath, tempCi.cid + ".xml",true);
                        }
                        xml = GlobalConfig.loadFullFileFromSaveFolder("novel", tempCi.cid + ".xml");
                    }
                        ...
                        for (int i = 0; i < nc.size(); i++) {
                            if (nc.get(i).type == 'i') {
                                ...
                                if (!LightCache.testFileExist(GlobalConfig.getFirstFullSaveFilePath()
                                    ...
                                    if (!LightNetwork.okHttpDownload(nc.get(i).content,null,GlobalConfig.getFirstFullSaveFilePath()
                                            + GlobalConfig.imgsSaveFolderName + File.separator,imgFileName,true)){

                                        if(!LightNetwork.okHttpDownload(nc.get(i).content,null,GlobalConfig.getSecondFullSaveFilePath()
                                                + GlobalConfig.imgsSaveFolderName + File.separator,imgFileName,true)){
    ...
    class AsyncRemoveBookFromCloud extends AsyncTask<Integer, Integer, Wenku8Error.ErrorCode> {
        ...
        protected Wenku8Error.ErrorCode doInBackground(Integer... params) {
            // params: aid
            byte[] bytes = LightNetwork.okHttpPostConnection(Wenku8API.getBaseURL(), Wenku8API.getDelFromBookshelfParams(params[0]));
        ...
        protected void onPostExecute(Wenku8Error.ErrorCode err) {
            ...
            if(err == Wenku8Error.ErrorCode.SYSTEM_1_SUCCEEDED) {
                ...
                if(fabFavorate != null)
                    fabFavorate.setIcon(R.mipmap.ic_favorate);
            }
            else
