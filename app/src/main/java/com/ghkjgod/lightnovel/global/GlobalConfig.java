package com.ghkjgod.lightnovel.global;
...
//        volleyRequestQueue = new RequestQueue(volleyCache, volleyNetwork);
    ...
    public static void loadAllSetting() {
        ...
        for (String set : sets) {
            ...
            if (temp.length != 2 || temp[0] == null || temp[0].length() == 0 || temp[1] == null || temp[1].length() == 0)
                continue;
    ...
    public static boolean saveNovelContentImage(String url) {
        ...
        if (!LightCache.testFileExist(getFirstFullSaveFilePath() + imgsSaveFolderName + File.separator + imgFileName)
            ...
            if (!LightNetwork.okHttpDownload(url, null, GlobalConfig.getFirstFullSaveFilePath()
                    + GlobalConfig.imgsSaveFolderName + File.separator, imgFileName, true)) {

                if (!LightNetwork.okHttpDownload(url, null, GlobalConfig.getSecondFullSaveFilePath()
                        + GlobalConfig.imgsSaveFolderName + File.separator, imgFileName, true)) {
                    return false;
